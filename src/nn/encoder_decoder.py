...
from src.nn.attention_pre_made import AttentionLayer
...
class BahdanauAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        def calculate_context_vec(step_inputs, states):
            ...
            print("Shape of step attention_weights", step_attention_weights)
            print("shape of step encoder_output", encoder_output)


            step_context_vector = step_attention_weights * encoder_output
            print("Step context_vector before reduce sum", step_context_vector)
...
def MainModel(text_max_len=1500,
    ...
    # ENCODER -----------------------------------------------------------------------------------------------------------#
    enc_input = tf.keras.layers.Input(shape=(text_max_len, ), dtype='int32', name='encoder_input')
    ...
    # enc_lstm_1, _, _ = tf.keras.layers.LSTM(
    ...
    # enc_lstm_2, _, _ = tf.keras.layers.LSTM(
    #     units=lstm_hidden_units, 
    #     return_sequences=True,
    #     return_state=True,
    #     dropout=0.2,
    ...
    # enc_lstm, enc_state_h, enc_state_c = tf.keras.layers.LSTM(
    #     units=lstm_hidden_units, 
    #     return_sequences=True,
    #     return_state=True,
    #     dropout=0.2,
    ...
    # DECODER -----------------------------------------------------------------------------------------------------------#
    dec_input = tf.keras.layers.Input(shape=(None,), name='decoder_input')
    ...
    attn_out, attn_states = AttentionLayer(name='attention_layer')([enc_lstm, dec_lstm])

    # Concatenation of context vector with decoder output
    dec_concat = tf.keras.layers.Concatenate(name='concat_hid_layer_context_vec')([dec_lstm, attn_out])
...
# def Encoder(trained_enc_dec_model,
#                 text_max_len=1500,
#                 enc_vocab_size=20000,
#                 embedded_dimension=128):
...
#     #     outputs=[
#     #         trained_enc_dec_model.get_layer('encoder_lstm').output # encoder_bidirectional_lstm
...
#     inf_enc_input = trained_enc_dec_model.get_layer('encoder_input').input
    
#     # enc_emb_weigths = trained_enc_dec_model.get_layer('encoder_embeddings').get_weights()[0]
#     # inf_enc_emb = tf.keras.layers.Embedding(
#     #     input_dim=enc_vocab_size,
#     #     output_dim=embedded_dimension,
#     #     input_length=text_max_len,
#     #     weights=[enc_emb_weigths],
#     #     trainable=False,
#     #     mask_zero=True
#     # )(inf_enc_input)

#     enc_emb = trained_enc_dec_model.get_layer('encoder_embeddings')
#     inf_enc_emb = enc_emb(inf_enc_input)

#     # inf_enc_lstm_layer_1 = trained_enc_dec_model.get_layer('encoder_lstm_1') 
#     # inf_enc_lstm_1, _, _ = inf_enc_lstm_layer_1(inf_enc_emb)

#     # inf_enc_lstm_layer_2 = trained_enc_dec_model.get_layer('encoder_lstm_2') 
#     # inf_enc_lstm_2, _, _ = inf_enc_lstm_layer_2(inf_enc_lstm_1)

#     # inf_enc_lstm_layer_3 = trained_enc_dec_model.get_layer('encoder_lstm_3') 
#     # enc_lstm, enc_state_h, enc_state_c = inf_enc_lstm_layer_3(inf_enc_lstm_2)

#     inf_enc_lstm_layer = trained_enc_dec_model.get_layer('encoder_lstm') 
#     enc_lstm, enc_state_h, enc_state_c = inf_enc_lstm_layer(inf_enc_emb, training=False)

#     inference_encoder = tf.keras.models.Model(
#         [inf_enc_input],
#         [enc_lstm, enc_state_h, enc_state_c]
#     )

#     return inference_encoder
...
def InferenceDecoder(trained_enc_dec_model,
    ...
    inf_decoder_hidden_states_input = tf.keras.layers.Input(shape=(text_max_len, lstm_hidden_units), name='inf_input_1')
    inf_decoder_state_input_h = tf.keras.layers.Input(shape=(lstm_hidden_units,), name='inf_input_2')
    inf_decoder_state_input_c = tf.keras.layers.Input(shape=(lstm_hidden_units,), name='inf_input_3')

    
    # load embedding layer
    # dec_emb_weigths = trained_enc_dec_model.get_layer('decoder_embeddings').get_weights()[0]
    # inf_dec_emb = tf.keras.layers.Embedding(
    #     input_dim=dec_vocab_size,
    #     output_dim=embedded_dimension,
    #     input_length=summary_max_len,
    #     weights=[dec_emb_weigths],
    #     trainable=False,
    #     mask_zero=True
    # )(inf_dec_input, training=False)

    # dec_emb = trained_enc_dec_model.get_layer('decoder_embeddings')
    # inf_dec_emb = dec_emb(inf_dec_input, training=False)
    ...
    inf_attention_layer = trained_enc_dec_model.get_layer('attention_layer')
    inf_context_vec, att_states = inf_attention_layer([inf_decoder_hidden_states_input, inf_dec_lstm], training=False)

    # Concatenation of context vector with decoder output
    inf_dec_concat = tf.keras.layers.Concatenate()([inf_dec_lstm, inf_context_vec])

    # A dense softmax layer to generate prob dist. over the target vocabulary
    inf_dense_layer = trained_enc_dec_model.get_layer('time_distributed') # time_distributed_dense
