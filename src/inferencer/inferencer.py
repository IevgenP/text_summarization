...
def decode_sequence(input_sequence, encoder, decoder_model, word_index, reverse_word_index, summary_max_len):
    ...
    # print("Encoder output", enc_hidden_state.shape)
    # print("Encoder output", enc_cell_state.shape)
    # print(enc_output[:,-1,:] == enc_hidden_state)
    ...
    while not stop_condition:
        #print('starting word: {}, starting word index: {}'.format(reverse_word_index[target_seq[0, 0]], target_seq[0, 0]))
        output_tokens, h, c = decoder_model.predict([target_seq] + [enc_output, input_hidden_state, input_cell_state])
        #print(output_tokens.shape, h.shape, c.shape)
        
        # Sample a token
        sampled_token_index = np.argmax(output_tokens[0, -1, :])
        sampled_token = reverse_word_index[sampled_token_index]
        #print('sampled_token: {}, sampled token index: {}'.format(sampled_token, sampled_token_index))

        decoded_sentence += ' ' + sampled_token

        # if(sampled_token!='sampleend'):
        #     decoded_sentence += ' ' + sampled_token
